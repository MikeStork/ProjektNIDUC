...
error_prob_bad: float = 0.01
state_change_prob_good_to_bad: float = 0.45
state_change_prob_bad_to_good: float = 0.45
...
for n in range(2, 128, 10):
    ...
    for z in range(10):
        ...
        for enc_block in signal_with_crc:
            ...
            success = checksums.CRC8(new_enc_block[:-8]) == new_enc_block[-8:]
            while success == False:
                new_enc_block = gec.transmit(enc_block)
                # number_of_errors += ber(enc_block, new_enc_block)
...
with open("wyniki_ok/wynikiCRC8GBCzly.txt", "w+") as f:
